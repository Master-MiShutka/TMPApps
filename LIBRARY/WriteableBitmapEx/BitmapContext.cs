 ...
 struct BitmapContext : IDisposable
    {
        ...
        public BitmapContext(WriteableBitmap writeableBitmap, ReadWriteMode mode)
        {
            ...
            this._pixelWidth = this.writeableBitmap.PixelWidth;
            this._pixelHeight = this.writeableBitmap.PixelHeight;
#if WPF
            ...
            double width = this._writeableBitmap.BackBufferStride / WriteableBitmapExtensions.SizeOfArgb;
            this._length = (int)(width * this._pixelHeight);
      ...
      public void Dispose()
      {
        ...
        {
            [System.Runtime.TargetedPatchingOptOut("Candidate for inlining across NGen boundaries for performance reasons")]
            get { return this._writeableBitmap.Format; }
        }
        ...
        {
            // Decrement the update count. If it hits zero
            if (DecrementRefCount(this._writeableBitmap) == 0)
            {
                // Remove this bitmap from the update map 
                UpdateCountByBmp.Remove(this._writeableBitmap);
