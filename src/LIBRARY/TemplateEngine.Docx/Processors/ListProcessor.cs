namespace TemplateEngine.Docx.Processors
{
    ...
    internal class ListProcessor : IProcessor
    {
        ...
        private class Prototype
        {
            ...
            {
                return new Prototype(this._context, this.PrototypeItems
            ...
            {
                return new LevelPrototype(this._context, this.PrototypeItems, fieldNames);
            }
        }
        ...
        private class LevelPrototype
        {
            ...
            private List<string> SdtTags => this.PrototypeItems == null
                        ? new List<string>()
                        : this.PrototypeItems.DescendantsAndSelf(W.sdt)
                        .Select(sdt => sdt.SdtTagName())
                        .ToList();

            public LevelPrototype Clone()
            {
                return new LevelPrototype(this._context, this.PrototypeItems.ToList());
            }
        }
        ...
        private ProcessResult FillContent(XElement contentControl, IContentItem item)
        {
            ...
            else
            {
                ...
                new NumberingAccessor(this.context.Document.NumberingPart, this.context.LastNumIds)
                    .ResetNumbering(prototype.PrototypeItems);

                // Propagates a prototype.
                if (list.Items != null)
                {
                    var propagationResult = this.PropagatePrototype(prototype, list.Items);
            ...
            IEnumerable<ListItemContent> content)
        {
            ...
            foreach (var contentItem in content)
            {
                ...
                {
                    var filledNestedFields = this.PropagatePrototype(
