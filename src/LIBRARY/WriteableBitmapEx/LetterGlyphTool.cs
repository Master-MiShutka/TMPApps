namespace System.Windows.Media.Imaging
{
    ...
    public static class LetterGlyphTool
    {
        ...
        public static unsafe void DrawLetter(this BitmapContext context, int x0, int y0, IntRect cliprect, System.Windows.Media.Color fontColor, GrayScaleLetterGlyph glyph)
        {
            if (glyph.Items == null)
            {
                return;
        ...
        public static int DrawString(this WriteableBitmap bmp, int x0, int y0, IntRect cliprect, System.Windows.Media.Color fontColor, GlyphFont font, string text)
        {
            return DrawString(bmp, x0, y0, cliprect, fontColor, null, font, text);
        }

        public static int DrawString(this WriteableBitmap bmp, int x0, int y0, IntRect cliprect, System.Windows.Media.Color fontColor, System.Windows.Media.Color? bgColor, GlyphFont font, string text)
        {
            if (text == null)
            {
                return 0;
        ...
        public static int DrawString(this WriteableBitmap bmp, int x0, int y0, IntRect cliprect, System.Windows.Media.Color fontColor, PortableFontDesc typeface, string text)
        {
            var font = GetFont(typeface);
            return bmp.DrawString(x0, y0, cliprect, fontColor, font, text);
        }

        public static int DrawString(this WriteableBitmap bmp, int x0, int y0, System.Windows.Media.Color fontColor, PortableFontDesc typeface, string text)
        {
            var font = GetFont(typeface);
            return bmp.DrawString(x0, y0, new IntRect(new IntPoint(0, 0), new IntSize(bmp.PixelWidth, bmp.PixelHeight)), fontColor, font, text);
        }

        public static int DrawString(this WriteableBitmap bmp, int x0, int y0, System.Windows.Media.Color fontColor, System.Windows.Media.Color? bgColor, PortableFontDesc typeface, string text)
        {
            var font = GetFont(typeface);
        ...
        public static GlyphFont GetFont(PortableFontDesc typeface)
        {
            ...
            {
                Typeface = new Typeface(new Media.FontFamily(typeface.FontName),
    ...
    public struct ColorGlyphKey : IEquatable<ColorGlyphKey>
    {
        ...
        public ColorGlyphKey(System.Windows.Media.Color fontColor, System.Windows.Media.Color backgroundColor, char @char)
        {
            this.FontColor = (fontColor.A << 24) | (fontColor.R << 16) | (fontColor.G << 8) | fontColor.B;
            this.BackgroundColor = (backgroundColor.A << 24) | (backgroundColor.R << 16) | (backgroundColor.G << 8) | backgroundColor.B;
            this.Char = @char;
        }

        public bool Equals(ColorGlyphKey other)
        {
            return this.FontColor.Equals(other.FontColor) && this.BackgroundColor.Equals(other.BackgroundColor) && this.Char == other.Char;
        }
        ...
        public override bool Equals(object obj)
        {
            ...
            return obj is ColorGlyphKey && this.Equals((ColorGlyphKey)obj);
        }
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                var hashCode = this.FontColor.GetHashCode();
    ...
    public class GlyphFont
    {
        ...
        {
            lock (this.ColorGlyphs)
            {
                var key = new ColorGlyphKey(fontColor, bgColor, ch);
                ClearTypeLetterGlyph glyph;

                if (!this.ColorGlyphs.TryGetValue(key, out glyph))
                {
                    glyph = ClearTypeLetterGlyph.CreateGlyph(this.GlyphTypeface, this.Font, this.EmSize, ch, fontColor, bgColor);
                    this.ColorGlyphs[key] = glyph;
                }
        ...
        public int GetTextWidth(string text, int? maxSize = null)
        {
            ...
            foreach (var ch in text)
            {
                ...
                if (this.IsClearType)
                {
                    var letter = this.GetClearTypeLetter(ch, Colors.Black, Colors.White);
                    if (letter == null)
                    {
                        continue;
                    }
                ...
                {
                    var letter = this.GetGrayScaleLetter(ch);
                    if (letter == null)
                    {
                        continue;
                    }
        ...
        public int GetTextHeight(string text)
        {
            ...
            return lines * this.TextHeight;
        }

        public int TextHeight => (int)Math.Ceiling(this.GlyphTypeface.Height * this.EmSize * DpiDetector.DpiYKoef);
    }
}
