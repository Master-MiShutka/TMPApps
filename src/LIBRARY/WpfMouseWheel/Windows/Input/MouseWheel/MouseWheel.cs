namespace WpfMouseWheel.Windows.Input
{
    ...
    public partial class MouseWheel : NativeMotionSource
    {
        ...
        public override IMotionInfo PreTransmit(int timeStamp, int nativeDelta)
        {
            ...
            this.UpdateResolutionFrequency(nativeDelta);
            if (!Debugger.IsAttached)
            {
                if (this.Delay.TotalSeconds > 1)
                    this._activeTransferCase.Reset();
            }

            return info;
        }

        public INativeMotionTransferOutput this[double resolution] => this._activeTransferCase[resolution];

        public IMotionInfo MotionInfo => this._activeTransferCase.MotionInfo;

        public double Remainder => this._activeTransferCase.Remainder;
        ...
        public int NativeRemainder => this._activeTransferCase.NativeRemainder;
        ...
        public override int NativeResolutionFrequency => this._nativeResolutionFrequency;

        public override double NativeToNormalized(int value)
        {
            return -(double)value / this.NativeNotchFrequency;
        }

        public override int NormalizedToNative(double value)
        {
            return (int)Math.Round(-value * this.NativeNotchFrequency);
        }

        public MouseDevice MouseDevice => this._mouseDevice;

        public MouseWheelResolution ResolutionMode => this.NativeResolutionFrequency < this.NativeNotchFrequency ? MouseWheelResolution.High : MouseWheelResolution.Standard;

        public int NativeNotchFrequency
        {
            [DebuggerStepThrough]
            get => NativeNotchFrequencyDefault;
        }

        public IMouseWheelTransferCase ActiveTransferCase
        {
            get => this._activeTransferCase;

            private set
            {
                if (this._activeTransferCase == value)
                    return;
                this._activeTransferCase = value;
                if (this.ActiveTransferCaseChanged != null)
                    this.ActiveTransferCaseChanged(this, EventArgs.Empty);
            }
        }

        public double Resolution
        {
            get => (double)this.GetValue(ResolutionProperty);

            private set => this.SetValue(ResolutionPropertyKey, value);
        }

        private double GetResolution()
        {
            return (double)this.NativeNotchFrequency / this.NativeResolutionFrequency;
        }
        ...
        private void UpdateResolutionFrequency(int nativeDelta)
        {
            ...
            if (nativeDelta == this._nativeResolutionFrequency)
                return;  // no change
            nativeDelta %= this.NativeNotchFrequency;
            if (nativeDelta == 0)
                nativeDelta = this.NativeNotchFrequency;
            if (nativeDelta == this.NativeNotchFrequency)
            {
                if (this.Delay.TotalSeconds <= 1)
                    return;
            }
            ...
            {
                if (nativeDelta >= this._nativeResolutionFrequency &&
                  (nativeDelta % this._nativeResolutionFrequency) == 0 && // exact divisor
                  this.Delay.TotalSeconds <= 1)
                    return;
            }

            this.SetNativeResolutionFrequency(nativeDelta);
        }

        private void SetNativeResolutionFrequency(int value)
        {
            var previous = this._nativeResolutionFrequency;
            this._nativeResolutionFrequency = value;
            this.OnResolutionFrequencyChanged(previous, value);
        }

        private string GetName()
        {
            if (this.MouseDevice == Mouse.PrimaryDevice)
                return "Primary Mouse Wheel";
            else
                return string.Format("Mouse Wheel [{0}]", this.Id);
        }

        private void UpdateActiveTransferCase()
        {
            if (this.NativeResolutionFrequency >= 20)
                this.ActiveTransferCase = this._stdResTransferCase;
            else
                this.ActiveTransferCase = this._hiResTransferCase = new MouseWheelMultiShaftTransferCase(this);
            this.Next = this._activeTransferCase;
        }
