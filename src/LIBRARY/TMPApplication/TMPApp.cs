...
namespace TMPApplication
{
    ...
    public partial class TMPApp : Application
    {
        ...
        public void MakeMenu(Window mainWindow)
        {
            ...
            IntPtr hMenu = TMP.Shared.Windows.SystemMenu.AddSysMenuSubMenu();
            if (hMenu != IntPtr.Zero)
            {
                ...
                {
                    TMP.Shared.Windows.SystemMenu.AddSysMenuSubItem(this.VisualThemesList[i].ShortName, index, VISUAL_THEME_BASE_ID + index, hMenu);
                    index++;
                }

                // Now add to main system menu (position 6)
                TMP.Shared.Windows.SystemMenu.AddSysMenuItem("Визуальная тема", 0, 6, hMenu);
                TMP.Shared.Windows.SystemMenu.AddSysMenuItem("-", 0, 7, IntPtr.Zero);
            }
        ...
        private IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            ...
            if (msg == (int)TMP.Shared.Windows.SystemMenu.WindowMessages.wmSysCommand)
            {
