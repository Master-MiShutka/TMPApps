    ...
    public class BalansGrop : PropertyChangedBase
    {
        ...
        public BalansGrop(Balans.IBalansGroup group, DatePeriod period)
        {
            if (group is Balans.SubstationSection || group is Balans.Substation)
            {
                ...
                IList<Balans.IBalansItem> fiders = group.Items.Where(i => i.ElementType == ElementTypes.FIDER).ToList();
                IList<Balans.IBalansItem> powerTransformers = group.Items.Where(i => i.ElementType == ElementTypes.POWERTRANSFORMER).ToList();
                IList<Balans.IBalansItem> auxiliary = group.Items.Where(i => i.ElementType == ElementTypes.UNITTRANSFORMERBUS).ToList();
