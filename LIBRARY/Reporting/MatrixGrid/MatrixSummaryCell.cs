namespace TMP.UI.Controls.WPF.Reporting.MatrixGrid
{
    ...
    {
        MatrixSummaryCell(object value, string contentFormat = null) : base(value, contentFormat)
        {
            CellType = MatrixCellType.SummaryCell;
        }
        ...
        {
            return string.Format(string.Format("MatrixSummary - {{0}} [{{1:{0}}}], R[{{2}}], C[{{3}}], RS[{{4}}, CS[{{5}}]", ContentFormat), SummaryType, Value, GridRow, GridColumn, GridRowSpan, GridColumnSpan);
        }
    }
}
