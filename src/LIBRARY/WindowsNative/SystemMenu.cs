namespace WindowsNative
{
    ...
    public static class SystemMenu
    {
        ...
        private static extern bool apiDrawMenuBar(IntPtr hWnd);

        [DllImport("user32.dll", EntryPoint = "GetSystemMenu", CharSet = CharSet.Unicode, SetLastError = true, ExactSpelling = true,
           CallingConvention = CallingConvention.Winapi)]
        private static extern IntPtr apiGetSystemMenu(IntPtr hWnd, bool bRevert);

        [DllImport("user32.dll", EntryPoint = "AppendMenuW", CharSet = CharSet.Unicode, SetLastError = true, ExactSpelling = true,
           CallingConvention = CallingConvention.Winapi)]
        private static extern bool apiAppendMenu(IntPtr hMenu, MenuFlags uFlags, uint uIDNewItem, string lpNewItem);

        [DllImport("user32.dll", EntryPoint = "InsertMenuItemW", CharSet = CharSet.Unicode, SetLastError = true, ExactSpelling = true,
           CallingConvention = CallingConvention.Winapi)]
        private static extern bool apiInsertMenuItem(IntPtr hMenu, uint uItem, bool fByPosition, [In] ref MENUITEMINFO lpmii);
        ...
        [DllImport("user32.dll", EntryPoint = "GetMenuItemInfoW", CharSet = CharSet.Auto, SetLastError = true, ExactSpelling = true,
           CallingConvention = CallingConvention.Winapi)]
        private static extern bool apiGetMenuItemInfo(IntPtr hMenu, uint uItem, bool fByPosition, ref MENUITEMINFO lpmii);

        [DllImport("USER32", EntryPoint = "CreatePopupMenu", SetLastError = true, CharSet = CharSet.Unicode, ExactSpelling = true,
           CallingConvention = CallingConvention.Winapi)]
        private static extern IntPtr apiCreatePopupMenu();
        ...
        public static IntPtr Handle => new WindowInteropHelper(Application.Current.MainWindow).Handle;
    }
}
